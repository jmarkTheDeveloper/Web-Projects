...
// UserSession is browser-only — guard prevents crash in Node.js
const activeSession = typeof window !== 'undefined' ? new UserSession() : null;
...
const AuthService = {
     ...
     *
     * UPDATED FEATURES:
     *  1. Easter Egg detection — returns easterEgg:true for special account
     *  2. Institutional Auto-Login — if an institutional email is not found,
     *     auto-creates their account so they never need to register manually
     *
     * @param {Object} params
     * @param {string} params.email
     * @param {string} params.password
     * @returns {Promise<{ token: string, user: Object, easterEgg?: boolean }>}
     */
    async login({ email, password }) {
        ...
        // Retrieve user record — also fetch is_easter_egg flag
        const [rows] = await db.execute(
            `SELECT id, name, email, password_hash, user_type, role, is_easter_egg
        ...
        // ── Institutional Auto-Login ────────────────────────────────────────
        // If an institutional email is not found in the database, automatically
        // create their account so they never need to manually register.
        if (rows.length === 0 && Validator.isInstitutionalEmail(cleanEmail)) {

            // Derive a display name from the email prefix
            // e.g. "mocklastnamexyz@national-u.edu.ph" → "Mocklastnamexyz"
            const emailPrefix = cleanEmail.split('@')[0];
            const autoName    = emailPrefix.charAt(0).toUpperCase() + emailPrefix.slice(1);

            // Validate password meets minimum requirements before auto-creating
            const passCheck = Validator.password(password);
            if (!passCheck.valid) {
                throw new AppError(
                    'Institutional accounts are created automatically on first login. ' +
                    `Your password must meet these requirements: ${passCheck.reason}`,
                    400,
                    'ERR_WEAK_PASSWORD'
                );
            }
            ...
            const [insertResult] = await db.execute(
                `INSERT INTO users
                 ...
                 VALUES (?, ?, ?, 'institutional', 'student', 0, NOW())`,
                [cleanEmail, autoName, passwordHash]
            );

            const newUserId = insertResult.insertId;
            const token     = jwt.sign(
                { userId: newUserId, userType: 'institutional', role: 'student' },
                JWT_SECRET,
        ...
        // If the user has is_easter_egg = 1, return a special flag.
        // The frontend detects this and shows the Easter Egg popup + audio.
        const isEasterEgg = user.is_easter_egg === 1;
        // ───────────────────────────────────────────────────────────────────
...
/*
   EXPORTS
   All items that other files need to import.
*/
...
module.exports = {
    ...
    // Session (browser only — null in Node.js)
    UserSession,
    activeSession,
